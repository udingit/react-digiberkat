...
import { View, ScrollView, TouchableOpacity, ActivityIndicator } from 'react-native';
import { useGetProductByIdQuery } from '@/src/store/api/productsApi';
import { useAddToCartMutation } from '@/src/store/api/cartApi';
...
import RestockRequestModal from '@/components/RestockRequestModal'; // Import modal yang baru dibuat
...
export default function ProductDetailsScreen() {
  ...
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  // State untuk varian yang dipilih. Awalnya null untuk memastikan pemilihan jika ada varian.
  const [selectedVariant, setSelectedVariant] = useState<any | null>(null);
  const [quantity, setQuantity] = useState(1);
  const [maxStock, setMaxStock] = useState(1); // Default 1, akan diupdate di useEffect

  // State untuk mengontrol visibilitas modal restock
  const [showRestockModal, setShowRestockModal] = useState(false);

  useEffect(() => {
    if (product) {
      // Jika produk memiliki varian, inisialisasi selectedVariant ke varian pertama
      // HANYA JIKA selectedVariant masih null (misal, saat pertama kali memuat produk)
      if (product.is_varians && product.variants && product.variants.length > 0 && selectedVariant === null) {
        setSelectedVariant(product.variants[0]);
      } else if (!product.is_varians) {
        // Jika tidak ada varian, pastikan selectedVariant adalah null
        setSelectedVariant(null);
      }

      // Update max stock berdasarkan selectedVariant atau product stock
      const stock = selectedVariant?.stock ?? product.stock ?? 1;
      setMaxStock(stock);
  ...
  const handleAddToCart = async () => {
    ...
    // Validasi tambahan untuk varian sebelum menambahkan ke keranjang
    if (product.is_varians && !selectedVariant) {
        toast.show({
            placement: 'top',
            render: ({ id: toastId }) => (
                <Toast nativeID={`toast-${toastId}`} className="px-5 py-3 gap-4 shadow-soft-1 items-center flex-row bg-yellow-500">
                    <ToastTitle size="sm" className="text-white">
    ...
    try {
      ...
      toast.show({
        ...
        render: ({ id: toastId }) => (
          <Toast
            nativeID={`toast-${toastId}`}
            className="px-5 py-3 gap-4 shadow-soft-1 items-center flex-row bg-green-500"
          >
            <Icon as={CheckCircle} size="xl" className="fill-white stroke-none" />
            <ToastTitle size="sm" className="text-white">
              Produk berhasil ditambahkan ke keranjang!
  ...
  // Handler untuk membuka modal restock
  const handleOpenRestockModal = () => {
    // Pastikan varian sudah dipilih jika produk memiliki varian
    if (product?.is_varians && !selectedVariant) {
        toast.show({
            placement: 'top',
            render: ({ id: toastId }) => (
                <Toast nativeID={`toast-${toastId}`} className="px-5 py-3 gap-4 shadow-soft-1 items-center flex-row bg-yellow-500">
                    <ToastTitle size="sm" className="text-white">
                        Pilih varian produk terlebih dahulu untuk meminta restock!
                    </ToastTitle>
                </Toast>
            ),
        });
  ...
  // Tentukan apakah tombol "Minta Penambahan Stok Produk" harus dinonaktifkan
  // Dinonaktifkan jika produk memiliki varian tetapi belum ada varian yang dipilih
  const isRestockButtonDisabled = product.is_varians && !selectedVariant;
  ...
  return (
    <ScrollView className="flex-1">
        ...
        <Card className="p-5 rounded-lg max-w-[960px] w-full">
          ...
          <Box className="flex-col sm:flex-row">
            <Button
              className="px-4 py-2 mr-0 mb-3 sm:mr-3 sm:mb-0 sm:flex-1"
              onPress={handleAddToCart}
              disabled={maxStock <= 0 || isAddingToCart || (product.is_varians && !selectedVariant)} // Disable jika stok habis, sedang menambahkan, atau varian belum dipilih
            >
              <ButtonText size="sm">
                {isAddingToCart ? (
                  <ActivityIndicator color="#fff" />
            ...
            <Button
              ...
              onPress={handleOpenRestockModal} // Panggil handler baru
              disabled={isRestockButtonDisabled} // Gunakan kondisi disable baru
            >
              <ButtonText size="sm" className="text-typography-600">
                Minta Penambahan Stok Produk
              </ButtonText>
      ...
      {/* Render RestockRequestModal secara kondisional */}
      {showRestockModal && product && (
        <RestockRequestModal
          isOpen={showRestockModal}
          onClose={() => setShowRestockModal(false)}
          productId={product.id}
          productVariantId={selectedVariant?.id || null} // Kirim ID varian yang dipilih, atau null jika tidak ada varian
          productName={product.name}
          variantName={selectedVariant?.name} // Kirim nama varian jika ada
        />
      )}
    </ScrollView>
  );
